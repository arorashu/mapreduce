...
func doMap(
	...
	for _, kv := range kvArray {
		var outFileHash = int(ihash(kv.Key)) % nReduce

		enc := jsonEncoders[outFileHash]
		err = enc.Encode(&kv)
	...
	//
	//println("kvArray : key: ", kvArray[10].Key, "value: " , kvArray[10].Value)
	//println("kvArray : key: ", kvArray[20].Key, "value: " , kvArray[20].Value)
	//println("kvArray size: ", len(kvArray))
	//
